...
const HolidayPackageAdd = () => {
  ...
  useEffect(() => {
    const draft = localStorage.getItem("goimomi_package_add_draft");
    if (draft) {
      try {
        const parsed = JSON.parse(draft);
        if (parsed.formData?.title || parsed.formData?.description || parsed.itineraryDays?.length > 1 || parsed.itineraryDays?.[0]?.title) {
  ...
  const handleDiscardDraft = () => {
    localStorage.removeItem("goimomi_package_add_draft");
    setShowRestorePrompt(false);
  };
  ...
  useEffect(() => {
    if (!isAutosaveEnabled) {
      localStorage.setItem("goimomi_package_add_autosave_enabled", "false");
      return;
    }
    localStorage.setItem("goimomi_package_add_autosave_enabled", "true");
    ...
    const saveTimeout = setTimeout(() => {
      ...
      try {
        ...
        localStorage.setItem("goimomi_package_add_draft", JSON.stringify(draftData));
  ...
  }, [formData, packageDestinations, highlights, inclusions, exclusions, cancellationPolicies, itineraryDays, fixedDepartureData, currentPage, isAutosaveEnabled]);
  const totalPages = 5;
    ...
    <div className="flex bg-[#fcfdfc] h-screen overflow-hidden font-outfit">
      ...
      <div className="flex-1 flex flex-col h-full overflow-hidden">
        ...
        <div className="flex-1 flex h-full overflow-hidden relative bg-[#fcfdfc] flex-col lg:flex-row">
          ...
          {/* Form Content Area */}
          <div className="flex-1 overflow-y-auto px-4 md:px-8 py-6 custom-scrollbar bg-[#fcfdfc]">
            <div className="max-w-4xl mx-auto pb-12">
              {/* Restore Draft Banner */}
              {showRestorePrompt && (
                <div className="p-6 bg-green-50 border-2 border-green-100 rounded-[2rem] flex flex-col md:flex-row justify-between items-start md:items-center gap-4 animate-in fade-in slide-in-from-top-4 duration-300 shadow-sm mb-6">
                  <div className="flex items-start gap-3">
                    <div className="bg-green-100 p-2.5 rounded-2xl text-xl shrink-0">📝</div>
                    <div>
                      <h4 className="font-black text-xs text-[#14532d] uppercase tracking-wide">Unsaved Draft Found</h4>
                      <p className="text-[10px] text-green-700/80 font-bold mt-1">We found a previously unsaved package draft in your browser. Would you like to restore your progress?</p>
                    </div>
                  </div>
