...
const RegionEdit = () => {
  ...
  const fetchInitialData = async () => {
    try {
      ...
      setFormData({
        ...
        country: regionRes.data.country
      });
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setLoading(true);
      await api.put(`/api/regions/${id}/`, formData);
  ...
  const handleDelete = async () => {
    if (!window.confirm("Are you sure you want to delete this region? All related cities will be affected.")) return;
    try {
      setLoading(true);
      ...
      alert("Failed to delete region. It might be in use.");
    } finally {
      setLoading(false);
    }
  };

  if (fetching) return <div className="flex bg-[#fcfdfc] h-screen items-center justify-center font-outfit text-[#14532d] uppercase font-black text-[10px] animate-pulse">Syncing Region State...</div>;
    ...
    <div className="flex bg-[#fcfdfc] h-screen overflow-hidden font-outfit text-[11px] font-bold">
      ...
      <div className="flex-1 flex flex-col h-full overflow-hidden text-gray-900 uppercase">
        ...
        <div className="bg-white border-b border-gray-100 px-6 py-4 flex justify-between items-center z-10 shadow-sm transition-all text-gray-900 uppercase tracking-tight">
          <div>
            <h1 className="text-lg font-black tracking-tighter uppercase">Modify Region Record</h1>
            <p className="text-[8px] text-gray-400 font-black uppercase tracking-[0.3em] leading-none mt-1 flex items-center gap-2">
              <span className="text-green-600">Master Data</span> / <span className="text-gray-900">Regional Cluster</span>
            </p>
          </div>
            ...
            <form onSubmit={handleSubmit} className="bg-white rounded-[1.5rem] border border-gray-100 shadow-xl overflow-hidden">
              <div className="p-8 space-y-6">
                ...
                <div className="pt-4 flex gap-3 text-white">
                  ...
                  >
                    <Save size={14} />
                    {loading ? "SAVING Changes..." : "UPDATE REGION Record"}
                  </button>
                  <button
                    type="button"
                    onClick={() => navigate(-1)}
