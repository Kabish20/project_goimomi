...
const CabBookingManage = () => {
    ...
    const handleDownloadVoucher = async (booking) => {
        try {
            const response = await api.get(`/api/cab-bookings/${booking.id}/download-voucher/`, {
                responseType: 'blob'
            });
            ...
            link.href = url;
            const bId = booking.booking_id || `CAB-${String(booking.id).padStart(5, '0')}`;
            link.setAttribute('download', `GoImomi_Voucher_${bId}.pdf`);
            ...
            alert("Failed to download voucher from server. Please try again.");
        }
    };
