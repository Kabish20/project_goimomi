...
const RegionManage = () => {
  ...
  const filtered = regions.filter(r => 
    ...
    r.country_name?.toLowerCase().includes(searchTerm.toLowerCase())
  );
    ...
    <div className="flex bg-[#fcfdfc] h-screen overflow-hidden font-outfit">
      ...
      <div className="flex-1 flex flex-col h-full overflow-hidden">
        ...
        <div className="bg-white border-b border-gray-100 px-6 py-3 flex justify-between items-center z-10 shadow-sm">
          <div>
            <h1 className="text-lg font-black text-gray-900 tracking-tighter uppercase">Region Management</h1>
          </div>
          <div className="flex gap-2">
            <button onClick={() => fetchRegions()} className="w-8 h-8 flex items-center justify-center rounded-full bg-gray-50 text-gray-400 hover:bg-green-50 hover:text-green-600 transition-all border border-gray-100">
              <RefreshCcw size={14} className={loading ? "animate-spin" : ""} />
            </button>
            <button onClick={() => navigate("/admin/management-country/regions/add")} className="px-4 py-2 rounded-full bg-[#14532d] text-white text-[9px] font-black uppercase tracking-[0.2em] shadow-lg shadow-green-900/20 hover:scale-105 transition-all flex items-center gap-2">
                        ...
                        <tr key={item.id} className="group hover:bg-[#fcfdfc] transition-all">
                          <td className="px-6 py-2.5">
                            <div className="flex items-center gap-3">
                              <div className="w-8 h-8 rounded-lg bg-purple-50 flex items-center justify-center text-purple-600 border border-purple-100/50 shadow-sm shrink-0">
                                <Layers size={14} />
                              </div>
                          ...
                          <td className="px-6 py-2.5 text-right">
                             <div className="flex items-center justify-end gap-1 text-gray-400">
                                <button 
                                  onClick={() => navigate(`/admin/management-country/regions/edit/${item.id}`)}
