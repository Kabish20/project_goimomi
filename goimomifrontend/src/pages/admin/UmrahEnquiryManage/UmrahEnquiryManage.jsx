...
import api from "../../../api";
import { Search, Eye, Trash2, Mail, Phone, Calendar, MapPin, Edit, Save } from "lucide-react";
...
const UmrahEnquiryManage = () => {
  ...
  const fetchEnquiries = async () => {
    try {
      setLoading(true);
      const response = await api.get(`${API_BASE_URL}/umrah-form/`);
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this umrah enquiry?")) {
      try {
        setLoading(true);
        await api.delete(`${API_BASE_URL}/umrah-form/${id}/`);
        fetchEnquiries();
      } catch (err) {
