...
import { days, exclusions, importantNotes, inclusions, peakPeriods, rupees, trip } from './baliData';
...
export default function Bali() {
  ...
  return <div className="trending-page bali-page">
    ...
    <nav className="bl-nav" aria-label="Bali package sections"><div className="td-container">{[['bl-overview', 'Overview'], ['bl-itinerary', 'Itinerary'], ['bl-stay', 'Stay & price'], ['bl-inclusions', 'Inclusions & exclusions'], ['bl-notes', 'Travel notes'], ['bl-enquire', 'Enquire now']].map(([id, title]) => <a href={`#${id}`} key={id}>{title}</a>)}</div></nav>
    ...
    <section id="bl-notes" className="td-container td-section"><span className="td-eyebrow">BEFORE YOU TRAVEL</span><h2>Travel period & <em>important notes.</em></h2><div className="bl-inclusions"><article><h3>Low Season 2026</h3><p className="bl-fine">This package applies during Low Season 2026, excluding the following peak travel periods:</p><ul>{peakPeriods.map(period => <li key={period}><CalendarDays size={15} />{period}</li>)}</ul><p className="bl-fine">Rates may vary during peak dates, festive periods and special events. Share your preferred dates for a confirmed quotation.</p></article><article><h3>Important notes</h3><ul>{importantNotes.map(item => <li key={item}><Check size={15} />{item}</li>)}</ul></article></div></section>
